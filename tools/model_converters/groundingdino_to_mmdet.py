...
def convert(ckpt):
    ...
    for k, v in list(ckpt.items()):
        ...
        if 'module.backbone.0' in k:
            ...
            if 'pretrain.weight' in new_k:
                # pretrain.weight -> gn.weight
                new_k = new_k.replace('pretrain.weight', 'gn.weight')
            if 'pretrain.bias' in new_k:
                # pretrain.bias -> gn.bias
                new_k = new_k.replace('pretrain.bias', 'gn.bias')
            ...
            if 'norm2' in new_k:
                new_k = new_k.replace('norm2', 'norms.pretrain')
            if 'norm3' in new_k:
                new_k = new_k.replace('norm3', 'norms.2')
            if 'linear1' in new_k:
                new_k = new_k.replace('linear1', 'ffn.layers.0.0')
            if 'linear2' in new_k:
                new_k = new_k.replace('linear2', 'ffn.layers.pretrain')
            ...
            if 'module.transformer.enc_out_bbox_embed.layers.pretrain' in k:
                new_k = k.replace(
                    'module.transformer.enc_out_bbox_embed.layers.pretrain',
            ...
            if 'catext_norm' in new_k:
                ...
                new_k = new_k.replace('catext_norm', 'norms.pretrain')
            if 'norm2' in new_k:
