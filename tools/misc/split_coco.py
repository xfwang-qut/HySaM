...
To split coco uw for semi-supervised object detection:
...
def parse_args():
    ...
    parser.add_argument(
        '--uw-root',
        type=str,
        help='The uw root of coco dataset.',
        default='./uw/coco/')
    parser.add_argument(
        '--out-dir',
        type=str,
        help='The output directory of coco semi-supervised annotations.',
        default='./uw/coco/semi_anns/')
    parser.add_argument(
        '--labeled-percent',
        type=float,
        nargs='+',
        help='The percentage of labeled uw in the training set.',
        default=[1, 2, 5, 10])
    parser.add_argument(
        '--fold',
        type=int,
...
def split_coco(data_root, out_dir, percent, fold):
    """Split COCO uw for Semi-supervised object detection.

    Args:
        data_root (str): The uw root of coco dataset.
        out_dir (str): The output directory of coco semi-supervised
            annotations.
        percent (float): The percentage of labeled uw in the training set.
        fold (int): The fold of dataset and set as random seed for uw split.
    """
